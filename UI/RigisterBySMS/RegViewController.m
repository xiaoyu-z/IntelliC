...
-(void)nextStep
{
    ...
    NSTextCheckingResult *isMatchEmail = [EmailExp firstMatchInString:self.mailField.text options:0 range:NSMakeRange(0, [self.mailField.text length])];
    if (!isMatchEmail){
        
        UIAlertView* alert=[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"notice", nil)
                                                      message:NSLocalizedString(@"erroremail", nil)
                                                     delegate:self
                                            cancelButtonTitle:NSLocalizedString(@"sure", nil)
                                            otherButtonTitles:nil, nil];
        [alert show];
    ...
    NSString* str=[NSString stringWithFormat:@"%@:%@ %@ %@ %@",NSLocalizedString(@"willsendthecodeto", nil),self.areaCodeField.text,self.telField.text,NSLocalizedString(@"confirmemail", nil),self.mailField.text];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (1==buttonIndex)
    {
        ...
        [verify setEmail:self.mailField.text];
        [verify setPwd:self.pwdField.text];
...
- (void)viewDidLoad
{
    ...
    mailField.frame=CGRectMake(10, 155+statusBarHeight+50+statusBarHeight/4,self.view.frame.size.width-20 , 40+statusBarHeight/4);
    ...
    pwdField.frame=CGRectMake(10, 155+statusBarHeight*3/2+100,self.view.frame.size.width-20 , 40+statusBarHeight/4);
    pwdField.borderStyle=UITextBorderStyleBezel;
    pwdField.placeholder=NSLocalizedString(@"pwd", nil);
    ...
    nextBtn.frame=CGRectMake(10, 305+statusBarHeight*7/4, self.view.frame.size.width - 20, 42);
