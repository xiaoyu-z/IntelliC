...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (alertView==_alert3)
    {
        ...
        NSString* phoneNum = [@"+" stringByAppendingString:_areaCode];
        phoneNum = [phoneNum stringByAppendingString:_phone];
        newOwner.phone = phoneNum;
        newOwner.email = _email;
        NSLog(@"%@",phoneNum);
        //上传
        //解决等待时间乱跳的问题
        servlet* newUser = [[servlet alloc]init];
        
        [newUser.manager POST:@"http:/192.168.1.108:8001/api/new_user/" parameters:@{@"user_phone":phoneNum,@"pwd":_pwd,@"email":_email}success:^(AFHTTPRequestOperation *operation, id responseObject) {
            NSDictionary* result=(NSDictionary*)responseObject;
             NSLog(@"%@",result);
            if([(NSString*)[result objectForKey:@"message"] isEqualToString:@"new user"]){
            UIAlertView* alter = [[UIAlertView alloc]initWithTitle:@"success" message:@"Register success" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alter show];
            OwnerDAO * op = [OwnerDAO sharedManager];
            [op create:newOwner];
            UIStoryboard * mainboard=[UIStoryboard storyboardWithName:@"Main" bundle:nil];
                //self.window.inputView
            self.view.window.rootViewController.modalPresentationStyle = UIModalTransitionStyleFlipHorizontal;
            self.view.window.rootViewController = [mainboard instantiateInitialViewController];
                NSUserDefaults* userDefaults = [NSUserDefaults standardUserDefaults];
                [userDefaults setBool:NO forKey:@"FirstTimeLoad"];
                //解决等待时间乱跳的问题
                [_timer2 invalidate];
                [_timer1 invalidate];
            }else{
                UIAlertView* alert=[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Fail", nil) message:[result objectForKey:@"message"] delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", nil) otherButtonTitles:NSLocalizedString(@"sure", nil), nil];
                [alert show];
            }
